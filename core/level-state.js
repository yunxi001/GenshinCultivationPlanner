...
export function parseLevelRange(value, label, max = 90) {
  ...
  const target = parseLevelToken(parts[1], `${label}目标等级`, max, true);
...
/** BetterGI 在突破临界等级以等级上限区分突破前后，例如 80/80 与 80/90。 */
export function resolveRecognizedAscension(level, levelLimit, label) {
  if (!isAscensionLevel(level)) return false;
  const parsedLimit = Number(levelLimit);
  if (!Number.isInteger(parsedLimit) || parsedLimit < level || parsedLimit > 90) {
    throw new Error(`${label}位于 ${level} 级，但等级上限识别结果无效，无法判断突破前后`);
  }
  return parsedLimit > level;
}

export function formatLevelState(level, ascended = false) {
  return isAscensionLevel(level)
